from ase.io import read 
import os

start_latticeparam = 9.52
end_latticeparam = 9.64
step_latticeparam = 0.02

start_ecut = 50
end_ecut = 80
step_ecut = 10

for ecut in  [round(start_ecut + i * step_ecut,2) for i in range(int((end_ecut - start_ecut) / step_ecut) + 1)]:
     if os.path.exists(f'../latticeparam/etot_ecut{ecut}_latticeparam.dat'):
        os.remove(f'../latticeparam/etot_ecut{ecut}_latticeparam.dat')
     for latticeparam in  [round(start_latticeparam + i * step_latticeparam,2) for i in range(int((end_latticeparam - start_latticeparam) /  step_latticeparam) + 1)]:
         
            structure = read(f'../latticeparam/alpha_{ecut}_{latticeparam}.out')
    
            with open(f'../latticeparam/etot_ecut{ecut}_latticeparam.dat', 'a') as f:
                f.write(f'{latticeparam} {structure.get_potential_energy()}\n')
